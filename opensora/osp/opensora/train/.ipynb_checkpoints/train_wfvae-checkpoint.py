...
class TrainingArguments:
    ...
    nii_frames: int = field(
        default=129, metadata={"help": "The number of frames per video."}
    )
    nii_hw: int = field(
        default=224, metadata={"help": "The number of frames per video."}
    )
    ...
    model_config: str = field(
        default="scripts/wfvae/wfvae.json",
...
def train(args):
    ...
    if args.load_from_checkpoint is not None:
        model = WFVAEModel.load_from_checkpoint(args.load_from_checkpoint)
        # model = CausalVAEModel.from_pretrained(args.load_from_checkpoint)
    else:
        model = WFVAEModel.from_config(args.model_config)

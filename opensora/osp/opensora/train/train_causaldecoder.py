...
from dataloader import NiiDataset, ImageDataset, MixedDataset
...
def train(args):
    ...
    decoder.add_condition()
    

    # model_decoder = model.decoder

    # decoder = ControlDecoder()
    # decoder.load_state_dict(model_decoder.state_dict(), strict=False)
    # decoder.post_quant_conv.load_state_dict(model.post_quant_conv.state_dict(), strict=True)
    # decoder.add_condition()
    ...
    ct_dataset = NiiDataset(
        folder_path = args.nii_path,
        frame=args.nii_frames,
        resolution=args.nii_hw,
    )

    print('len(ct_dataset)', len(ct_dataset))
    
    # i_dataset = ImageDataset(
    ...
    #         video_dataset=v_dataset,
    #         nii_dataset= ct_dataset,
    #         image_dataset= i_dataset,
    #     )
    ...
    #     # decoder = ControlDecoder.load_from_checkpoint(args.load_from_checkpoint)
    #     # model = CausalVAEModel.from_pretrained(args.load_from_checkpoint)
    #     trainer = pl.Trainer(resume_from_checkpoint=args.load_from_checkpoint)
    #     print('model loaded')
    # else:
    #     print('fail!!')
    #     exit()
...
# trainer.fit(model)
    ...
    trainer.fit(decoder, train_loader, **trainer_kwargs)

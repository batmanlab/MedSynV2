...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        hs = [self.conv_in(x)]
        # print('in encoder, hs', hs[0].shape)
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
                h = self.down[i_level].block[i_block](hs[-1])
                if len(self.down[i_level].attn) > 0:
                    h = self.down[i_level].attn[i_block](h)
                # print('in encoder, num_res_blocks', h.shape)
                hs.append(h)
...
class Decoder(VideoBaseAE_PL):
    ...
    def forward(self, z):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                ...
                # print('after up, ', i_level, h.shape)
            if hasattr(self.up[i_level], "upsample"):
                h = self.up[i_level].upsample(h)
                # print('after upsample, ', i_level, h.shape)
            if hasattr(self.up[i_level], "time_upsample"):
                h = self.up[i_level].time_upsample(h)
                # print('after time_upsample, ', i_level, h.shape)
    ...
    #     modules_to_train = [
    #         self.conv_in.named_parameters(),
    #         self.mid.named_parameters(),
    ...
    #     lowres = self.get_input(batch, 'lowres')
    #     feature = self.get_input(batch, 'feature')
    #     # inputs = 
    #     # lowres_all = torch.cat([lowres, feature], dim=1)
    #     reconstructions = self.forward(feature)

    #     np.save('reconstructions_1.npy', reconstructions.cpu().float().detach().numpy())
    #     exit()
...
class ControlDecoder(VideoBaseAE_PL) :
    def __init__(
        self,
        z_channels: int=4,
        hidden_size: int=128,
        ...
        conv_in: Module = "CausalConv3d",
        conv_out: Module = "CausalConv3d",
        attention: Module = "AttnBlock3D",
        resnet_blocks: Tuple[Module] = (
            "ResnetBlock3D",
            "ResnetBlock3D",
            "ResnetBlock3D",
        ...
        temporal_upsample: Tuple[Module] = ("", "", "TimeUpsample2x", "TimeUpsample2x"),
        mid_resnet: Module = "ResnetBlock3D",
        ...
        self.post_quant_conv = quant_conv_cls(4, 4, 1)
        if hasattr(self.loss, "discriminator"):
    ...
    def add_condition(self):
        self.conv_in_control = resolve_str_to_obj("Conv2d")(
            8, self.block_in, kernel_size=3, padding=1
        )
        self.mid_control = copy.deepcopy(self.mid)
        tem_block_2 = resolve_str_to_obj('ResnetBlock3D_cond')(in_channels = self.mid.block_2.in_channels, out_channels=self.mid.block_2.out_channels, dropout=0)
        tem_block_2.load_state_dict(self.mid_control.block_2.state_dict(), strict=False)
        self.mid_control.block_2 = tem_block_2
        self.mid_control.block_2.conv2 = zero_module(self.mid_control.block_2.conv2)

        self.up_control = copy.deepcopy(self.up[-1])
        self.up_control.upsample = zero_module(self.up_control.upsample)
        
        
    def forward(self, z, lowres):
        h = self.conv_in(z)
        h = self.mid.block_1(h)
        h = self.mid.attn_1(h)
        h = self.mid.block_2(h)

        if hasattr(self, 'conv_in_control'):
            h_control = self.conv_in_control(torch.cat([z, lowres], dim=1))
            h_control = self.mid_control.block_1(h_control)
            h_control = self.mid_control.attn_1(h_control)
            h_control = self.mid_control.block_2(h_control)
            h_temp = h+h_control

            for i_block in range(self.num_res_blocks + 1):
                h_temp = self.up_control.block[i_block](h_temp)
                if len(self.up_control.attn) > 0:
                    h_temp = self.up_control.attn[i_block](h_temp)
            if hasattr(self.up_control, "upsample"):
                h_temp = self.up_control.upsample(h_temp)
            if hasattr(self.up_control, "time_upsample"):
                h_temp = self.up_control.time_upsample(h_temp)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if hasattr(self, 'conv_in_control') and i_level==self.num_resolutions-1:
                h = h+h_temp
    ...
    def configure_optimizers(self):
        ...
        modules_to_train = [
            self.conv_in_control.named_parameters(),
            self.mid_control.named_parameters(),
            self.up_control.named_parameters(),
        ]
        params_with_time = []
        params_without_time = []
        for name, param in chain(*modules_to_train):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # inputs = 
        # lowres_all = torch.cat([lowres, feature], dim=1)
        reconstructions = self.forward(feature,  lowres)
...
class CausalVAEModel(VideoBaseAE_PL):
    ...
    def _training_step_gan(self, batch, batch_idx):
        ...
        reconstructions, posterior = self(inputs)
        # print('reconstructions', reconstructions.shape)
        # print('posterior', posterior.sample().shape)
        # exit()
        opt1, opt2 = self.optimizers()

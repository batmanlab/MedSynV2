...
class Trainer(object):
    def __init__(
        ...
        train_files = []
        items = [item.replace('.npy', '') for item in os.listdir(folder)[:]]
        ...
        self.means = [nothing_means, ]
        self.stds = [nothing_stds, ]

    def load(self, milestone, **kwargs):
        if milestone == -1:
            path = 'final_ckpt' #dirs[-1]
    ...
    def train(
        ...
        for i, data in enumerate(self.dl):
            with torch.no_grad():
                idx_feat = -1
                nothing_feat = torch.zeros(1, 4, 112, 56, 56).to(self.accelerator.device).half()
                chosen_feat = nothing_feat
